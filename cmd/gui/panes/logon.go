...
func LogonScreen(MyWin fyne.Window) fyne.CanvasObject {
	...
	webaddresslabel := widget.NewLabel(config.GetLangs("wa"))
	webaddress := widget.NewEntry()
	webaddress.SetPlaceHolder(config.GetLangs("wa"))
	webaddress.Disable()
	var webaddressShadow = ""

var adsmaxminutesShadow = ""
	...
	TPbutton := widget.NewButtonWithIcon(config.GetLangs("ls-trypass"), theme.LoginIcon(), func() {
		...
		if !iserrors {
			...
			dbaddressShadow = config.FyneApp.Preferences().StringWithFallback("DBADDRESS", config.Encrypt("db.newhorizons3000.org:5432/radio?sslmode=verify-ca", config.MySecret))
			dbaddress.SetText(config.Decrypt(dbaddressShadow, config.MySecret))
			config.FyneApp.Preferences().SetString("DBADDRESS", config.Encrypt(dbaddress.Text, config.MySecret))
			...
			dbuser.SetText(config.Decrypt(dbuserShadow, config.MySecret))
	...
	SSbutton := widget.NewButtonWithIcon(config.GetLangs("ls-title"), theme.LoginIcon(), func() {
		...
		if webaddressShadow != webaddress.Text {
			...
			if !haserrors {
				...
				errors.SetText(config.GetLangs("wa-err1"))
			}
