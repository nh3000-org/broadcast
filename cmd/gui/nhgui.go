...
func main() {
	...
	var Panes = map[string]Pane{
		...
		"users":      {config.GetLangs("ra-users"), "", theme.ListIcon(), panes.UsersScreen, true},
	...
	config.FyneMainWin.SetContent(container.NewAppTabs(
		...
		container.NewTabItemWithIcon(Panes["users"].Title, Panes["users"].Icon, panes.UsersScreen(config.FyneMainWin)),
